...
vec3 Camera::World_Position(const ivec2& pixel_index)
{
    vec3 result;
    // TODO
    double ax = (max[0] - min[0])/number_pixels[0];
    double ay = (max[1] - min[1])/number_pixels[1];
    double r = min[0]+ax*(pixel_index[0]+0.5);
    double s = min[1]+ay*(pixel_index[1]+0.5);
    result = film_position + r*horizontal_vector + s*vertical_vector;
    return result;
}
