...
Object* Render_World::Closest_Intersection(const Ray& ray, Hit& hit)
{
  ...
  std::vector<Hit> hits;
  int count = 0;
  hit.object = NULL;//std::cout << "size "<< objects.size() << '\n';
  for(int i =0; i< objects.size(); i++){
    if(objects[i]->Intersection(ray, hits)){
      if(count == 0){
        if(hits.back().ray_exiting){
          if(hits[hits.size()-2].t){
...
void Render_World::Render()
{
  ...
  for(int p=0;p<n; p++ )
  for(int q=0;q<n; q++ )
  Render_Pixel(ivec2(i+p/n,j+q/n));
}
...
vec3 Render_World::Cast_Ray(const Ray& ray,int recursion_depth)
{
  ...
  if(h.object != NULL && !(recursion_depth > recursion_depth_limit)){
    ...
    color += background_shader->Shade_Surface(ray, ray.endpoint, ray.direction, recursion_depth);
  }
  return color;
}
